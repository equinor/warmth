...
class Results:
    ...
    def sediment_density(self, age: int, sediment_id: int | None = None) -> resultValues:
        ...
        sed_poro["values"][start_idx:end_idx] = Forward_model._sediment_density(
            sed_poro["values"][start_idx:end_idx], bulk_density, 1000)
        return sed_poro
    ...
    def effective_conductivity(self, age: int, sediment_id: int | None = None) -> resultValues:
        ...
        """
        from .forward_modelling import Forward_model
        v = Forward_model._sediment_conductivity_sekiguchi(self.sediment_porosity(
            age)["values"], self._reference_conductivity(age), self.temperature(age)["values"])
        d = self.depth(age)
        d = (d[1:]+d[:-1])/2
        sed_id = self.sediment_ids(age)
        if isinstance(sediment_id, int):
    ...
    def _filter_sed_id_index(self, sed_id: int, sed_id_arr: np.ndarray) -> Tuple[int, int]:
        ...
        if sed_id in sed_id_arr:
            ...
            raise Exception(
                f"Invalid sediment id {sed_id}. Valid ids: {np.unique(sed_id_arr[~np.isnan(sed_id_arr)])}")
    ...
    def temperature_history(self, sed_id: int) -> np.ndarray[np.float64]:
        max_age_for_sed = np.max(np.argwhere(
            self._sediments_ids == sed_id)[:, 1])
        ...
        while cell_idx >= 0:
            for age in reversed(all_valid_sed_age):
                t_old = self._mid_pt_temperature(
                    self.temperature(age, sed_id)["values"])
    ...
    def vitrinite_reflectance(self, sed_id: int | None = None, Rotype="Easy%RoV") -> resultValues:
        ...
        if isinstance(sed_id, type(None)):
            ...
            v = np.empty(0)
            for i in only_sed:
                v_new = self.vitrinite_reflectance_history(int(i), Rotype)[
                    :, 0]
                v = np.append(v, v_new)
...
class VR:
    ...
    def cum_reacted(A, E, weights, time, temp_k):
        # 8.3144621/4184 (  Convert from kilocalorie to Joule by multiplication of 4184)
        c = 0.0019858775
        # Dimensions
        nt = temp_k.size
        nw = weights.size
        ...
        for i in range(1, time.shape[0]):
            heat_rate[i] = (
                (temp_k[i] - temp_k[i - 1]) /
                (time[i] - time[i - 1]) / 31600000000000
            )
    ...
    def easyRoV(
        temp_k: np.ndarray,
        dt_myr_between: np.ndarray,
        seconds_per_myr: float = 3.16e13,
        beta_tol_k_per_s: float = 1e-15,
    ) -> np.ndarray:
        """
        Cumulative reacted fraction using the Sweeney & Burnham ΔI/β scheme,
        vectorized across activation-energy components. Uses Burnham (2019)
        Easy%RoV parameters.

        Parameters
        ----------
        temp_k : np.ndarray
            Temperatures (K) per time step. Trailing NaNs are ignored.
        dt_myr_between : np.ndarray
            Time increments (Myr) between consecutive temperature steps.
            Length must be >= n_valid - 1.
        seconds_per_myr : float
            Conversion factor, default 3.16e13.
        beta_tol_k_per_s : float
            Minimum heating rate (K/s) to count a step as eligible.

        Returns
        -------
        RoV : np.ndarray
            Easy%RoV per step, same shape as temp_k.
        """
        nt = temp_k.size
        CR = np.zeros(nt, dtype=float)

        # Leading non-NaN portion
        n_valid = int(np.count_nonzero(~np.isnan(temp_k)))
        if n_valid == 0:
            return CR

        T = temp_k[:n_valid].astype(float)

        # Validate dt_myr
        if dt_myr_between.ndim != 1:
            raise ValueError(
                "dt_myr_between must be a 1D array of length len(temp_k)-1.")
        if dt_myr_between.size < max(n_valid - 1, 0):
            raise ValueError(
                f"dt_myr_between length {dt_myr_between.size} is shorter than "
                f"required {n_valid - 1} for the valid portion of temp_k."
            )

        # Extract matching dt (Myr), clamp negatives to 0
        dt_myr = dt_myr_between[: max(n_valid - 1, 0)].astype(float)
        dt_myr = np.where(dt_myr > 0.0, dt_myr, 0.0)

        # --- Constants (Burnham 2019 Easy%RoV) ---
        A = 2e15
        E = np.array(
            [40, 42, 44, 46, 48, 50, 52, 54, 56, 58,
            60, 62, 64, 66, 68, 70, 72, 74, 76],
        ...
        weights = np.array(
            [0.03, 0.04, 0.045, 0.045, 0.045, 0.04, 0.045, 0.05, 0.055, 0.06,
            0.07, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03, 0.03, 0.025],
            dtype=float,
        )
        c = 0.0019858775  # R in kcal/(mol·K) = 8.3144621 / 4184

        # --- dT and heating rate β (K/s) ---
        dT = np.diff(T, prepend=T[0])  # dT[0] = 0

        dt_seconds = np.zeros(n_valid)
        if n_valid > 1:
            dt_seconds[1:] = dt_myr * seconds_per_myr

        beta = np.zeros(n_valid)
        if n_valid > 1:
            # avoid divide-by-zero
            beta[1:] = dT[1:] / np.maximum(dt_seconds[1:], 1e-30)

        # --- Reheat / cooling skip ---
        run_max = np.maximum.accumulate(T)
        prev_max = run_max.copy()
        if n_valid > 1:
            prev_max[1:] = run_max[:-1]

        eligible = (beta > beta_tol_k_per_s) & (T >= prev_max)

        # --- I(T, E) temperature integral ---
        T2 = T[:, None]
        E2 = E[None, :]
        E_RT = E2 / (c * T2)

        num = E_RT**2 + 2.334733 * E_RT + 0.250621
        den = E_RT**2 + 3.330657 * E_RT + 1.681534

        I_valid = A * T2 * np.exp(-E_RT) * (1.0 - num / den)

        # --- ΔI accumulation ---
        dI = np.zeros_like(I_valid)
        dI[1:] = I_valid[1:] - I_valid[:-1]

        beta_safe = np.where(eligible, np.maximum(beta, 1e-30), 1.0)
        step_contrib = (dI / beta_safe[:, None]) * eligible[:, None].astype(float)

        DI_valid = np.cumsum(step_contrib, axis=0)

        # --- Weighted cumulative reacted → Ro ---
        CR_valid = (1.0 - np.exp(-DI_valid)) @ weights
        CR[:n_valid] = CR_valid

        RoV = 0.223 * np.exp(3.7 * CR)
        return RoV
