...
class UniformNodeGridFixedSizeMeshModel:
    ...
    def __init__(self, nodeGrid, nodes, modelName="test" ):
        self.node1D = nodes
        ...
        self.numElemInCrust = 0  # split crust hexahedron into pieces
        self.numElemInLith = 0  # split lith hexahedron into pieces
        self.numElemInAsth = 0  # split asth hexahedron into pieces
    ...
    def write_tetra_mesh_resqml( self, out_path):
        ...
        cond_per_cell = self.thermalCond.x.array.copy()
    ...
    def write_hexa_mesh_resqml( self, out_path):
        ...
        hexaHedra, hex_data_layerID = self.buildHexahedra()
        ...
        write_hexa_grid_with_properties(filename_hex, np.array(points_cached), hexa_renumbered, "hexamesh",
            np.array(T_per_vertex), np.array(age_per_vertex), poro0_per_cell, decay_per_cell, density_per_cell,
            np.array(cond_per_cell), rhp_per_cell, lid_per_cell)
        return filename_hex
    ...
    #
    def getSedimentPropForLayerID(self, property, layer_id):
        """
        """           
    ...
    def porosity0ForLayerID(self, layer_id):
        ...
        if (layer_id>=0) and (layer_id<self.numberOfSediments):
            phi = self.globalSediments.phi[layer_id]
            decay = self.globalSediments.decay[layer_id]
            return phi, decay
        return 0.0, 0.0

    def cRhoForLayerID(self, ss):
        #
        ...
        if (ss>=0) and (ss<self.numberOfSediments):
            rho = self.globalSediments.solidus[ss]
            return 1000*rho
        return 1000*self.parameters1D.crustsolid

    def kForLayerID(self, ss):
    ...
    def buildVertices(self, time_index=0, useFakeEncodedZ=False):
        ...
        for k in range(self.num_nodes):
            ...
            for ss in range(self.numberOfSediments):
                ...
                if (tti==51):
                    if (k==0) or (k==1100):
                        print(k, tti, ss, top_of_sediments, base_of_current_sediments, zpos, node.sed[:,:,tti])
    ...
    def buildHexahedra(self):
        ...
        return hexaHedra, hex_data_layerID
    ...
    def constructMesh(self):
        ...
        cells = []
        cell_data_layerID = []
        for h,lid in zip(hexaHedra, hex_data_layerID):
            for t in tetsplit0:
        ...
        mesh = meshio.Mesh(
            ...
            cell_data={"layer": [np.array(cell_data_layerID, dtype=np.float64)] },
        )
    ...
    def normalizedZ(self, z):
        return (z-self.Zmin) / (self.Zmax-self.Zmin)

    def TemperatureGradient(self, x):
        nz = self.normalizedZ(x[2,:])
    ...
    def updateSedimentsConductivity(self):
        ...
        for i,t in enumerate(tet):
            ...
            # conductivity_effective = 1.84 + 358 * ( (1.0227*conductivity_effective)-1.882) * ((1/temperature_K)-0.00068)
    ...
    def sedimentsConductivitySekiguchi(self): #mean_porosity, conductivity, temperature_C):
        ...
        for i,t in enumerate(tet):
            ...
            mean_porosity = f1*f2
            self.porosityAtDepth.x.array[i] = mean_porosity
            # cond_local = self.kForLayerID(lidval) * (1.0-mean_porosity)
            cond_local = self.kForLayerID(lidval) * (1.0-mean_porosity)
            temperature_C = np.mean(np.array([ self.uh.x.array[ti] for ti in t]))
            temperature_K = 273.15 + temperature_C
            conductivity_effective = 1.84 + 358 * ( (1.0227*cond_local)-1.882) * ((1/temperature_K)-0.00068)
            if (i==0):
                print("Sekiguchi at i=0", self.tti,  conductivity_effective, cond_local, self.kForLayerID(lidval), temperature_C, mean_porosity, poro0, top_km, bottom_km)
            if (bottom_km>3.9):
                if (i % 20 == 0):
                    print("Sekiguchi", i, self.tti,  conductivity_effective, cond_local, self.kForLayerID(lidval), temperature_C, mean_porosity, poro0, top_km, bottom_km)
                    print("Sekiguchi", i, top_km, bottom_km, f1, f2)
            # conductivity_effective = self.kForLayerID(lidval) ** (1 - mean_porosity) * 0.6 ** mean_porosity
            self.thermalCond.x.array[i] = conductivity_effective

        # conductivity = conductivity*mean_porosity
        # temperature_K=273.15+temperature_C
        # conductivity_effective = 1.84+358((1.0227*conductivity)-1.882)((1/temperature_K)-0.00068)
        # return conductivity_effective
    ...
    def buildKappaAndLayerIDs(self):
        ...
        ks = [ self.kForLayerID(lid) for lid in ls]
        thermalCond.x.array[:] = np.array(ks, dtype=PETSc.ScalarType).flatten()

        crs = [ self.cRhoForLayerID(lid) for lid in ls]
        ...
        decay = [ self.porosity0ForLayerID(lid)[1] for lid in ls]
    ...
    def writeKappaFunction(self, outfilename, tti=0):
        """ Writes the mesh and kappa function (constant value per cell) to the given output file in XDMF format
        """         
        import dolfinx
        xdmf = dolfinx.io.XDMFFile(MPI.COMM_WORLD, outfilename, "w")
        xdmf.write_mesh(self.mesh)
    ...
    def setupSolverAndSolve(self, time_step=-1, no_steps=100, skip_setup = False, initial_state_model = None):
        ...
        if (not skip_setup):
            ...
            self.FE = ufl.FiniteElement("CG", self.mesh.ufl_cell(), 1)
        ...
        if (self.thermalCond is None):
            self.thermalCond, self.c_rho, self.layerIDsFcn = self.buildKappaAndLayerIDs()
        ...
        f = dolfinx.fem.Constant(self.mesh, PETSc.ScalarType(0))  # source term; constant zero
        ...
        if ( self.useBaseFlux ):
            ...
            # domain_zero.x.array[  np.abs(self.mesh.geometry.x[:,2]) < 0.05 ] = 1
            ...
            # g = (-1.0*self.baseFluxMagnitude) / (self.parameters1D.asthsolid*1000) * ufl.conditional( x[2] > self.Zmax*0.99999, 1.0, 0.0 )
            # g = (-1.0*self.baseFluxMagnitude) / (self.parameters1D.asthsolid*1000) * ufl.conditional( domain_c > 0, 1.0, 0.0 )
            # g = (-1.0*baseFlux) * ufl.conditional( domain_c > 0, 1.0, 0.0 )
            g = (-1.0*baseFlux) * ufl.conditional( domain_c > 0, 1.0, 0.0 ) # - (1.0*baseFlux) * ufl.conditional( domain_zero > 0, 1.0, 0.0 )
